...
This Go script tests the latency (ping) and DNS resolution time for a list of popular DNS providers (both IPv4 and IPv6). It performs the following tasks:
...
- Measures average ping and DNS resolution time for a set of predefined domains.
...
- Linux/macOS/Windows environment (script uses `ping` and `dig` commands).
...
1. Clone the repository or download the script:
   ```bash
   git clone https://github.com/your-username/dns-tester.git
   cd dns-tester
